...
function getBiggerAndSmaller(num1, num2) {
  return num1 > num2
...
function multiplyBiggerNumByTwo(num1, num2) {
  const { bigger } = getBiggerAndSmaller(num1, num2);
  return bigger * 2;
}
...
function eatMostTacos(sum1, sum2) {
  const { bigger } = getBiggerAndSmaller(sum1, sum2);
  return `I ate ${bigger} tacos.`;
}

function adoptSmallerDog(weight1, weight2) {
  const { smaller } = getBiggerAndSmaller(weight1, weight2);
